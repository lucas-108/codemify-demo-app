...
async def run_test():
    ...
    try:
        ...
        # -> Send GET request to /products API endpoint to verify HTTP status and response body
        ...
        # -> Explicitly verify the HTTP status code of the GET request to /api/products is 200 OK
        await page.goto('http://localhost:3000/api/products', timeout=10000)
        await asyncio.sleep(3)
        ...
        await expect(frame.locator('text=Codemify Store').first).to_be_visible(timeout=30000)
        ...
        await expect(frame.locator('text=$15.99').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=© 2025 Codemify Store - Demo Application for Testing').first).to_be_visible(timeout=30000)
